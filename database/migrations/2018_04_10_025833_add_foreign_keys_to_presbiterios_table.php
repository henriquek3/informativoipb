...
class AddForeignKeysToPresbiteriosTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('presbiterios', function (Blueprint $table) {
            ...
            $table->foreign('usuario_inclusao', 'presbiterios_fk1')->references('id')->on('usuarios')->onUpdate('RESTRICT')->onDelete('RESTRICT');
            $table->foreign('usuario_alteracao', 'presbiterios_fk2')->references('id')->on('usuarios')->onUpdate('RESTRICT')->onDelete('RESTRICT');
        });
    }
