...
class AddForeignKeysToRelatoriosEstatisticasTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('relatorios_estatisticas', function (Blueprint $table) {
            ...
            $table->foreign('usuario_inclusao', 'relatorios_estatisticas_fk2')->references('id')->on('users')->onUpdate('RESTRICT')->onDelete('RESTRICT');
            $table->foreign('usuario_ultima_alteracao', 'relatorios_estatisticas_fk3')->references('id')->on('users')->onUpdate('RESTRICT')->onDelete('RESTRICT');
        });
    }
